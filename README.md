...
2. **Scraping** (`scrape.py`) — Downloads the detailed occupation sheets from the ISTAT website into `html/`.
3. **Parsing** (`parse_detail.py`, `process.py`) — BeautifulSoup converts the HTML into clean Markdown in `pages/`.
4. **Table** (`make_csv.py`) — Extracts structured fields (description, education, examples) into `professioni.csv`.
5. **Scoring** (`score.py`) — Sends each occupation to Claude (Haiku 4.5 via Anthropic SDK) with a scoring rubric. Output: `scores.json` with a 0-10 score and bilingual IT/EN rationale.
6. **Site data** (`build_site_data.py`) — Merges CSV and scores into `site/data.json`.
7. **Website** (`site/index.html`) — Bilingual interactive visualization with three views: treemap, sunburst, and bar chart.

## Exposure scale

| Score | Level | Examples |
|-------|-------|---------|
| 0-1 | Minimal | Bricklayer, gardener, diver |
| 2-3 | Low | Electrician, plumber, firefighter |
| 4-5 | Moderate | Nurse, police officer, veterinarian |
| 6-7 | High | Teacher, manager, accountant |
| 8-9 | Very high | Software developer, translator, data analyst |
| 10 | Maximum | Data entry clerk, telemarketing operator |

## Data sources

- **Occupations:** [ISTAT CP2021](https://professioni.istat.it/sistemainformativoprofessioni/cp/) — Italian Classification of Occupations 2021
